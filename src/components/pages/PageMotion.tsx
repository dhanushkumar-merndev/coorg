  ...
  useLayoutEffect(() => {
    ...
    media.add({ motion: "(prefers-reduced-motion: no-preference)", compact: "(max-width: 700px)" }, (context) => {
      ...
      const animation = gsap.context(() => {
        ...
        entry.fromTo("[data-chapter-entry]", entryFrom, {
          x: 0, y: 0, yPercent: 0, scale: 1, opacity: 1, duration: 1.1, stagger: 0.12, ease: "power3.out",
        });
        entry.fromTo("[data-chapter-intro]", { y: 20, opacity: 0 }, {
        ...
        gsap.utils.toArray<HTMLElement>("[data-chapter-words]").forEach((heading) => {
          ...
          const words = heading.querySelectorAll("[data-chapter-word]");
          if (treatment === "ink") {
            gsap.fromTo(words, { opacity: 0.22 }, {
              opacity: 1, stagger: 0.16, ease: "none",
              scrollTrigger: { trigger: heading, start: "top 92%", end: "bottom 62%", scrub: true },
            });
          } else if (treatment === "wipe") {
            ...
            gsap.fromTo(heading, { x: compact ? 12 : 32, opacity: 0.2 }, {
        ...
        gsap.utils.toArray<HTMLElement>("[data-chapter-image]").forEach((image) => {
          ...
          gsap.fromTo(image, { yPercent: -distance / 2 }, {
        ...
        gsap.utils.toArray<HTMLElement>("[data-chapter-rise]").forEach((element) => {
          gsap.fromTo(element, { y: compact ? 24 : 48, opacity: 0 }, {
